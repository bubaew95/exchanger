 ...
 */
final class TelegramOptions extends AbstractTelegram
{
    ...
    public function mediaGroup(array $media) : static
    {
        $this->options['media_group']['media'] = array_map(
            fn(AbstractMedia $photo) => $photo->toArray(), $media
        );

        foreach ($this->options['media_group']['media'] as $key => $item) {
            $this->options[$item['media_item']] = fopen(realpath($item['media_item']), 'r');
            unset($this->options['media_group']['media'][$key]['media_item']);
        }
